...
export default function BatchesList() {
  return (
    <AppShell breadcrumbs={<span><Link to="/dashboard" className="hover:underline">Dashboard</Link> · Batches</span>}>
      <div className="px-6 py-6 max-w-[1400px]">
        ...
        <div className="panel">
          <div className="flex flex-wrap items-center gap-2 px-3 py-2 border-b border-border">
            ...
            {["All status", "Marketplace: Amazon.eg", "Owner: Anyone", "Last 30 days"].map((f) => (
              <button key={f} className="h-8 px-2.5 rounded-sm border border-border bg-background text-xs text-foreground hover:bg-muted inline-flex items-center gap-1.5">
                ...
                {batches.map((b) => (
                  <tr key={b.id} className="hover:bg-muted/30">
                    <td className="px-4 py-3">
                      <Link to={`/batches/${b.id}/review`} className="font-medium hover:underline">{b.name}</Link>
                      <div className="text-[11px] text-muted-foreground label-mono">{b.id} · {b.marketplace}</div>
                    </td>
                    ...
                    <td className="px-2 py-3 text-right tabular-nums text-status-ready">{b.ready}<span className="text-muted-foreground"> + {b.readyAugmented}</span></td>
                    ...
                    <td className="px-2 py-3"><SubmissionChip state={b.submission} /></td>
        ...
        {/* Empty state preview note for ops */}
