...
export function OrganizationProvider({ children }: PropsWithChildren) {
  ...
  const { isLoaded: isListLoaded, organizationList, setActive } = useOrganizationList();
  ...
  const value = useMemo<OrganizationContextValue>(() => {
    ...
    const workspaces =
      organizationList?.map((entry) => ({
        ...
        membershipRole: entry.membership.role ?? null,
      })) ?? [];
  ...
  }, [isListLoaded, isOrgLoaded, membership?.role, organization, organizationList, setActive]);
