...
export default function IntakeMapping() {
  return (
    <AppShell breadcrumbs={<span><Link to="/batches" className="hover:underline">Batches</Link> · AW25 Wave 3 · Intake mapping</span>}>
      <div className="px-6 py-5 max-w-[1400px]">
        <div className="flex items-end justify-between mb-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Confirm how source columns map to internal fields before processing continues. Nothing is submitted yet.</p>
          </div>
          <div className="flex items-center gap-2">
            <Link to="/batches/new" className="h-9 px-3 rounded-sm border border-border bg-card text-sm hover:bg-muted inline-flex items-center gap-1.5">
              <ArrowLeft className="h-4 w-4" /> Back
            </Link>
            <Link to={appPaths.batchReview(prototypeBatchId)} className="h-9 px-3 rounded-sm bg-primary text-primary-foreground text-sm font-medium hover:bg-primary-hover inline-flex items-center gap-1.5">
        ...
        <div className="grid lg:grid-cols-[1.5fr_1fr] gap-4">
          <section className="panel">
            <header className="flex items-center justify-between px-4 py-3 border-b border-border">
              ...
              <button className="text-xs text-primary hover:underline inline-flex items-center gap-1"><Edit2 className="h-3 w-3"/>Edit all</button>
            </header>
                  ...
                  {mappings.map((m) => {
                    ...
                    return (
                      <tr key={m.target} className={low ? "bg-status-needs-input-bg/40" : ""}>
                        ...
                        <td className="px-2 py-2.5"><code className="text-[11px] bg-muted px-1.5 py-0.5 rounded-sm">{m.target}</code></td>
                        ...
                        <td className="px-2 py-2.5 text-right">
                          {m.confidence === 0
                            ...
                            : <span className={`tabular-nums text-xs ${low ? "text-status-needs-input" : "text-foreground"}`}>{(m.confidence * 100).toFixed(0)}%</span>}
                        </td>
                      </tr>
                    );
                  })}
